...
from stt_mapf.grid import Grid
...
class Planner:
    def __init__(self,
        ...
        self.static_obstacles = KDTree(grid.static_obstacles)
    ...
    def plan(self, start: Tuple[int, int],
        ...
        # Assume dynamic obstacles are agents with same radius, distance needs to be 1*radius
        def safe_dynamic(grid_pos: np.ndarray, time: int) -> bool:
            nonlocal dynamic_obstacles
            return all(self.l2(grid_pos, obstacle) >= self.robot_radius
        ...
        def safe_semi_dynamic(grid_pos: np.ndarray, time: int) -> bool:
            ...
            for timestamp, obstacles in semi_dynamic_obstacles.items():
                flag = True
                if time >= timestamp:
                    flag = all(self.l2(grid_pos, obstacle) >= self.robot_radius for obstacle in obstacles)
                if not flag:
                    return False
            return True
        ...
        while open_set and iter_ < max_iter:
            ...
            if current_state.pos_equal_to(goal):
                if debug:
                    ...
                    print('Path of agent ', came_from[next(iter(came_from))].pos, ':\n', self.reconstruct_path(came_from, current_state))
                    # print('Actual time elapsed (stochastic environment): ', self.calculate_actual_time(came_from, current_state))
    ...
    '''
    def calculate_actual_time(self, came_from: Dict[State, State], current: State) -> np.ndarray:
        total_traverse_time = 0
        total_vertex_delay = 0
        while current in came_from.keys():
            current = came_from[current]
            delay = self.grid.sample_vertex_delay(current.pos)
            total_traverse_time += 1
            total_vertex_delay += delay
        return total_traverse_time + total_vertex_delay
