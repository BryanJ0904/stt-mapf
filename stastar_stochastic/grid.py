...
class Grid:
    ...
    def __init__(
        ...
        grid_size: int,
        static_obstacles: np.ndarray,
        n_v: Optional[Union[float, np.ndarray]] = 1.0,
        lambda_v: Optional[Union[float, np.ndarray]] = 1.0,
        ...
        # RNG for stochastic delay sampling
        self.rng = np.random.default_rng(rng_seed)

        # Gamma parameters per vertex
        self.n_v = self._initialize_param_map(n_v, "n_v")
        self.lambda_v = self._initialize_param_map(lambda_v, "lambda_v")
    ...
    def sample_vertex_delay_by_index(self, i: int, j: int) -> float:
        """
    ...
    def sample_delay_map(self) -> np.ndarray:
        ...
        return self.rng.gamma(shape=self.n_v, scale=scale_map)
