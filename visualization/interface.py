...
# from stt_cbs.planner import Planner as STTCBSPlanner
#
#
...
#             lambda_v=3.0,
#         )
#         planner = STTCBSPlanner(grid=mapf)
...
class Interface:
    def __init__(self, scenario_path: str, paths: List[np.ndarray] = None):
        ...
        self.starts = [tuple(p) for p in args["start_coordinates"]]
        self.goals = [tuple(p) for p in args["goal_coordinates"]]
        self.static_obstacles = [tuple(p) for p in args["static_obstacles"]]
    ...
    def compute_grid_dimension(self) -> int:
        max_coord = 0
        for p in self.starts + self.goals + self.static_obstacles:
            max_coord = max(max_coord, p[0], p[1])
        return max_coord + 1
    ...
    def make_demo_paths(self) -> List[np.ndarray]:
        return [np.array([s for _ in range(5)], dtype=int) for s in self.starts]
    ...
    def cell_top_left(self, x: int, y: int) -> Tuple[int, int]:
        return self.margin + x * self.cell_px, self.margin + y * self.cell_px
    ...
    def draw_grid(self, frame: np.ndarray) -> None:
        ...
        for i in range(self.n + 1):
            ...
            cv2.line(frame, (x, self.margin),
                     (x, self.margin + self.n * self.cell_px), (200, 200, 200), 1)

            cv2.line(frame, (self.margin, y),
                     (self.margin + self.n * self.cell_px, y), (200, 200, 200), 1)

    def draw_starts_goals(self, frame: np.ndarray) -> None:
        for i, (x, y) in enumerate(self.starts):
            cv2.circle(frame, self.cell_center(x, y),
                       max(3, self.cell_px // 6),
                       self.agent_colors[i], -1)
        ...
        for i, (x, y) in enumerate(self.goals):
            ...
            p0 = (tl[0] + pad, tl[1] + pad)
            p1 = (tl[0] + self.cell_px - pad, tl[1] + self.cell_px - pad)
            cv2.rectangle(frame, p0, p1, self.agent_colors[i], 2)
    ...
    def draw_agents(self, frame: np.ndarray, step: int) -> None:
        for i, path in enumerate(self.paths):
            ...
            cv2.circle(frame, center,
                       ...
                       self.agent_colors[i], 2)

            cv2.putText(frame, str(i),
                        (center[0] - 8, center[1] + 6),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.5, self.agent_colors[i], 1)

    def draw_info(self, frame: np.ndarray, step: int) -> None:
        text = f"Step: {step}/{max(0, self.max_steps - 1)}   Press q to quit"
    ...
    # Planner
    # ------------------------------------------------------------
    def plan_with_stt_cbs(self, grid):
        from stt_cbs.planner import Planner as STTCBSPlanner

        planner = STTCBSPlanner(grid=grid)
        paths = planner.plan(self.starts, self.goals, epsilon=0.01, debug=False)
    ...
    def plan_with_mappo_model(self):
        from marllib import marl
        from marllib.examples.mapf import RLlibMAPF, Evaluate
        ...
        paths = Evaluate.render_paths(
            ...
            model,
            restore_path={
                "params_path": r"C:\Users\Lenovo\Documents\PyCharm\mapf\marllib\examples\exp_results\mappo_mlp_custom\MAPPOTrainer_mapf_custom_7e0ec_00000_0_2026-05-07_14-58-11\params.json",
                "model_path": r"C:\Users\Lenovo\Documents\PyCharm\mapf\marllib\examples\exp_results\mappo_mlp_custom\MAPPOTrainer_mapf_custom_7e0ec_00000_0_2026-05-07_14-58-11\checkpoint_000100\checkpoint-100"
            },
            local_mode=True,
            num_workers=0
        )
